...
export default function EmployeePortalPage() {
  ...
  const getPayslipDeductions = (payslip: PayslipRow) => {
    const savedTotal = Number(payslip.total_deductions ?? payslip.deductions ?? 0);

    const rebuiltTotal =
      Number(payslip.late_deduction || 0) +
      Number(payslip.undertime_deduction || 0) +
      Number(payslip.absence_deduction || payslip.absent_deduction || 0) +
      Number(payslip.cash_advance || 0) +
      Number(payslip.other_deductions || 0) +
      Number(payslip.manual_deduction || 0) +
      Number(payslip.balance_deduction || 0) +
      Number(payslip.sss_deduction || 0) +
      Number(payslip.philhealth_deduction || 0) +
      Number(payslip.pagibig_deduction || 0) +
  ...
  const getPayslipGross = (payslip: PayslipRow) => {
    const savedGross = Number(payslip.gross_pay ?? payslip.total_earnings ?? 0);
    ...
    return Math.max(savedGross, rebuiltGross);
  };

  const getPayslipNet = (payslip: PayslipRow) => {
    const savedNet = Number(
      payslip.net_pay ?? payslip.final_pay ?? payslip.release_amount ?? 0
    );

    if (savedNet > 0) return savedNet;

    return getPayslipGross(payslip) - getPayslipDeductions(payslip);
  };
  ...
  const getPayslips = async (employeeId: string) => {
    const employeeNoRaw = String(
      currentUser?.employee_no || currentUser?.employee_number || employeeNumber || ""
    ).trim();

    const employeeNoCandidates = Array.from(
      new Set(
        [
          employeeNoRaw,
          employeeNoRaw && !employeeNoRaw.toUpperCase().startsWith("BIO-")
            ? `BIO-${employeeNoRaw.padStart(3, "0")}`
            : "",
          employeeNoRaw.toUpperCase().startsWith("BIO-")
            ? employeeNoRaw.replace(/^BIO-/i, "")
            : "",
        ].filter(Boolean)
      )
    );

    const employeeName = `${currentUser?.first_name || ""} ${
      currentUser?.last_name || ""
    }`.trim();

    const queries = [
      { column: "employee_id", value: employeeId },
      ...employeeNoCandidates.map((value) => ({
        column: "employee_no",
        value,
      })),
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 rounded-xl border border-slate-800 bg-slate-900/70 p-3 text-xs text-slate-400">
                      Source: Payroll Snapshot • {payslip.snapshot_type || "Released"}
                    </div>
                  </div>
                );
              })}
