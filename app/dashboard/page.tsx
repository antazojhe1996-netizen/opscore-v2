...
export default function ExecutiveDashboardPage() {
  ...
  const sharedExpenses = sumExpensesByUnit([
    "shared",
    "admin",
    "electric",
    "water",
  ...
  const criticalAlerts = [
    ...(cashFlowStatus === "Critical"
      ? [`Cash flow is critical. Projected shortage: ${formatPeso(Math.abs(projectedCashPosition))}.`]
      : []),
    ...(cashFlowStatus === "Watch"
      ? [`Cash flow needs monitoring. Projected position: ${formatPeso(projectedCashPosition)}.`]
      : []),
    ...(overdueBills.length > 0
      ? [`${overdueBills.length} overdue bill(s) worth ${formatPeso(overdueBillsTotal)} need payment review.`]
      : []),
    ...(upcomingBills.length > 0
      ? [`${upcomingBills.length} bill(s) due within 14 days.`]
      : []),
    ...(payrollRatio >= 50
      ? [`Payroll ratio is high at ${payrollRatio}%. Target is 35%-40%.`]
      : []),
    ...(payrollRatio >= 40 && payrollRatio < 50
  ...
  const recommendations = [
    ...
    ...(payrollRatio >= 40
      ...
      : []),
    ...(potentialRecovery > 0
      ? [`Recovering pending balances can improve cash position by ${formatPeso(potentialRecovery)}.`]
      : []),
    ...(Math.abs(totalVariance) > 0
