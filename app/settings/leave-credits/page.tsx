...
export default function LeaveCreditsPage() {
  ...
  const selectedEmployee = employees.find(
    (employee) =>
      String(employee.employee_no) === String(selectedEmployeeNo)
  );
  ...
  const loadEmployeeCredits = (employeeNo: string) => {
    ...
    leaveSettings.forEach((leave) => {
      const existingCredit = leaveCredits.find(
        (item) =>
  ...
  const saveAllCredits = async () => {
    ...
    for (const leave of leaveSettings) {
      ...
      const existingCredit = leaveCredits.find(
        (item) =>
          String(item.employee_no) === String(selectedEmployeeNo) &&
          item.leave_type === leave.leave_type
      );
  ...
  const deleteLeaveCredit = async (id: number) => {
    const confirmDelete = confirm(
      "Are you sure you want to delete this leave credit record?"
    );
      ...
      <main className="flex-1 p-6">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400">
            Configure all leave credits per employee in one setup table.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <section className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-lg">
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-400">
                Enter credits for each enabled leave type.
              </p>
            </div>
                  ...
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr className="border-b border-slate-800 text-left text-slate-400">
                        ...
                        <th className="py-3 pr-4">Current Used</th>
                        <th className="py-3 pr-4">Current Remaining</th>
                      </tr>
                    </thead>
                    ...
                    <tbody>
                      {leaveSettings.map((leave) => {
                        const existingCredit = leaveCredits.find(
                          (item) =>
                            String(item.employee_no) ===
                              String(selectedEmployeeNo) &&
                            item.leave_type === leave.leave_type
                        );
          ...
          >
            ...
            {leaveSettings.map((leave) => {
              const credit = leaveCredits.find(
                (item) =>
                  String(item.employee_no) === String(employee.employee_no) &&
                  item.leave_type === leave.leave_type
              );
