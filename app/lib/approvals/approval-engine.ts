 ...
 * - optional employee balance creation
...
async function checkExecutionLock(requestId: string) {
...
async function createExecutionLock(requestId: string, companyId: string) {
...
async function deleteExecutionLock(requestId: string) {
  await supabase
    .from("cash_execution_locks")
    .delete()
    .eq("source_document_id", requestId);
...
export async function executeApproval(context: ApprovalExecutionContext) {
  const {
    request,
    currentEmployeeName,
    currentEmployeeId,
  ...
  const lock = await checkExecutionLock(request.id);
  ...
  await createExecutionLock(request.id, finalCompanyId);
  ...
  try {
    ...
    await deleteExecutionLock(request.id);
    throw error;
  }
}

/**
 * Backward-compatible export for existing cash approval imports.
 */
export async function executeCashDrawerApprovalAction(
  context: ApprovalExecutionContext,
) {
  return executeApproval(context);
}